...
| Pi 会话与配置 | 真进程 new A / new B / prompt A 会话隔离、load A 更新凭据，参数特殊字符完整保留 |
| Kimi 隔离 | 配置不回写用户源、会话跨进程保留、无效配置错误不泄漏内容 |
| 连续对话输入稳定性 | Go 测试验证受管问答 MCP 跨端点保持固定服务名和工具身份，同时轮换随机端口与鉴权；Ally Runtime 矩阵验证 resume 只发送本轮新增消息 |
...
```sh
npm install --prefix /tmp/acp-pi-deps @earendil-works/pi-coding-agent@0.84.1 @modelcontextprotocol/sdk@1.30.0
go build -o /tmp/acp-agent ./cmd/acp-agent
node scripts/pi-integration/run.mjs /tmp/acp-pi-deps /tmp/acp-agent
```

脚本在新临时目录隔离 HOME、Pi 配置和会话，运行本地假 OpenAI provider 及 MCP 服务，退出后打印结果与产物目录。Go 直连真实 Pi，验证 15 次审批、6 次实际 MCP 调用、29 次本地模型请求；另覆盖模型/思考切换、list/load/resume/close/delete、文件差异、终端退出、input/editor、模板和七个无需模型压缩的内置命令。`/compact` 的指令传递与队列/settled 屏障由 Go RPC fixture 验证；它验证真实 Pi 执行路径，不代表商业模型质量或真实账号登录通过。完整配置含测试凭据，所有值均为假数据。
...
Cursor 官方 2026.09.02-c22c1a3 下载返回 HTTP 403，真实 CLI 测试未完成。macOS x64 是实测平台；Windows 批处理启动、符号链接及进程树回收尚未实测。Pi input/editor 要求客户端 form elicitation；auto/read-only 沙箱不在本接入支持范围。浏览器和桌面视觉验收未执行。
