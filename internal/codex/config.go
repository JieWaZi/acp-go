...
// agentModes 返回与 upstream AgentMode 完全一致且顺序稳定的三个 V1 模式。
...
// Options 按 upstream 顺序构造 mode、model 与可用的 reasoning_effort ACP 配置。
...
// selectModel 应用模型选择，并按照 upstream 规则保留或回退 reasoning effort。
...
// capitalize 复用 upstream 对 effort 展示名进行首字母大写的规则。
