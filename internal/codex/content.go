...
func buildPromptInputElement(block acp.ContentBlock) (protocol.InputElement, error) {
	...
	case block.Audio != nil:
		// 与 upstream 静默丢弃 audio 的行为不同：V1 未声明支持时必须显式失败。
...
// textInput 构造 app-server 需要的文本 input，并保留上游的空 text_elements 数组。
...
// embeddedResourceInput 将文本、图片或二进制资源转换为 upstream 使用的输入表现。
...
// isSupportedImageURL 仅允许 upstream 支持的 HTTP、HTTPS 与 data URL。
