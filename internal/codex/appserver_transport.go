...
// appServerTransport 实现固定 codex-acp StdUtils.ts 的无 jsonrpc NDJSON 边界。
// 与上游 vscode-jsonrpc 不同之处仅是 Go 语言边界和显式有界资源策略。
...
// 该时序等价于 CodexJsonRpcConnection.startCodexConnection 的 connection.listen()。
...
// readLoop 按 newline 拆帧；对应 upstream StdUtils.createJSONRPCReader。
...
func (t *appServerTransport) dispatchLine(line []byte) {
	...
	if err := json.Unmarshal(line, &envelope); err != nil {
		// 固定 upstream 明确忽略 malformed 行；仅写 stderr logger 便于诊断。
