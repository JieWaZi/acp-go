...
const (
	// approvalAllowOnce 是基础单次授权 option id。
	approvalAllowOnce acp.PermissionOptionId = "allow_once"
	// approvalAllowAlways 是基础 Session 授权 option id。
	approvalAllowAlways acp.PermissionOptionId = "allow_always"
	// approvalRejectOnce 是基础拒绝 option id。
...
// commandPermissionRequest 按稳定顺序构造基础选项和可选策略修订选项。
...
// execpolicyAmendmentLabel 生成命令策略修订标签。
...
// networkPolicyAmendmentLabel 按 allow/deny 生成网络策略标签。
...
// networkPolicyAmendmentDescription 生成 common permission 网络规则说明。
...
// baseApprovalOptions 返回三个基础选项及其 Codex decision 元数据。
...
func permissionGrantOption(
	...
	// 没有任何 common permission change 时省略整个 permission 扩展。
...
// permissionSummary 生成审批工具卡片展示的权限文本。
