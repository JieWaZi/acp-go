...
// trackPendingURLElicitation 保存已接受且等待服务端完成信号的 URL 交互。
...
// rawMetaRecord 把开放 `_meta` 安全转换为 ACP map，非对象按空元数据处理。
...
// marshalRawMeta 把 ACP 响应元数据转换为开放 JSON；空元数据保持省略。
