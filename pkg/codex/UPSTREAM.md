...
本文档是 Codex Adapter 唯一的上游追溯入口。协议声明、生成类型和运行时行为必须先回到这里列出的固定源码与 fixture 核对，不能以 README 或二手摘要替代源码。Claude 的独立基线见 [`../claude/UPSTREAM.md`](../claude/UPSTREAM.md)。
...
- 通知：turn/item 生命周期、已知但不直接转 ACP 的 `turn/diff/updated`、消息/reasoning/plan/usage、command/file/MCP 进度、MCP 启动状态、request resolved、compaction、model reroute、warning/error，以及登录流程依赖的 `AccountLoginCompletedNotification`。
...
| `src/CodexJsonRpcConnection.ts` 的 `startCodexConnection`、`attachLogs`，`CodexAcpServer.runWithProcessCheck` | `pkg/codex/process.go`、`appserver_transport.go`、`executable.go` | 唯一 `codex app-server` 进程、stdin/stdout、退出 dispose、调用方完整环境透传、实时 stderr 结构化日志与有界崩溃尾部；Go `Wait`/`StdoutPipe` closed-pipe 竞态只在 `FinalError` 已确认异常退出时升级为 exit code/stderr；可执行文件来源按本项目 V1 的 `CODEX_PATH`→PATH 约束替换 bundled npm fallback，调用方前置参数统一放在探测和 app-server 子命令之前 |
...
| `command-action-events.test.ts`、`file-change-events.test.ts` 及其 snapshots | `TestEventRouterMapsFileChangesToStandardDiff`、`TestEventRouterPreservesUnverifiableFileUpdate`、`file_diff_test.go`：parsed Command action、File add/delete rich diff、update 未应用/已应用补丁、move 目标文件与坏补丁降级 |
...
4. 对照 `CodexAppServerClient.ts`、`CodexAcpClient.ts`、`CodexEventHandler.ts` 实际 imports/dispatcher 更新 `protocol.root.json` 和 `envelope.go` 的集中方法清单；运行 `go generate ./pkg/codex/protocol`。
...
- 2026-08-25：把 `turn/diff/updated` 纳入强类型通知但按 upstream 保持无 ACP 输出；参考 `createPatchContent` 补齐 update/move 的当前文件读取、补丁正反应用与标准 ACP old/new diff，坏补丁继续安全降级为 raw change。
