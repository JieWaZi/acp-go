...
// TestEventRouterFallsBackToTerminalOutputOnCommandCompletion 锁定命令完成时的输出回退。
...
// assertMetaWire 把 SDK 扩展元数据编码为真实 JSON，并与手工 fixture 精确比较。
...
// TestEventRouterMapsParsedCommandActions 验证已解析的 read/search/listFiles 不伪装成终端。
