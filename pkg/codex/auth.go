...
var (
	// errAuthenticationFailed 是不包含服务端详情或凭据的认证失败哨兵错误。
...
// browserAuthEnabled 在 NO_BROWSER 为空时允许浏览器认证。
...
// waitForLoginCompletion 等待 typed 通知并返回不包含服务端错误文本的稳定错误。
...
// apiKeyFromMeta 读取 ACP `_meta.api-key.apiKey` 扩展。
