...
| `src/acp-agent.ts` `canUseTool`、`src/tools.ts` | `permission.go`、`events.go` | tool call mapper、`session/requestPermission`、严格 option 校验和异常 fail closed |
...
| `src/settings.ts` 的 permission mode 解析 | `config.go`、`launch.go` | 默认安全模式和 Session 启动参数 |
...
- fast 使用 ACP select `on/off`；危险 `bypassPermissions` 不在 V1 选项中，未知或错误模式不会扩大权限。
