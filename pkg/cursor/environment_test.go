...
func TestExecutionModelArgument(t *testing.T) {
	selection := CursorModelSelection{ModelID: "composer-2.5"}
	if got, err := executionModelArgument(selection); err != nil || got != "composer-2.5" {
		t.Fatalf("model: %q %v", got, err)
	}
	selection = CursorModelSelection{ModelID: "claude-opus-4-8", Parameters: []CursorModelParameter{{ID: "effort", Value: "high"}, {ID: "fast", Value: "false"}}}
