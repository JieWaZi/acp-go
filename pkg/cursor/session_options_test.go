...
func TestCursorExecutionSelection(t *testing.T) {
	...
	adapter := newCursorSessionAdapter()
	adapter.sessions["s"] = &cursorSessionOptions{nativeOptions: []acp.SessionConfigOption{
		option("mode", "agent", "mode"),
		option("model", "claude-sonnet-5", "model"),
		option("thinking", "true", "thought_level"),
		option("effort", "high", "thought_level"),
		option("fast", "false", "model_config"),
	}}
	selection, err := adapter.ExecutionModel("s")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = adapter.ExecutionModel("missing"); err == nil {
