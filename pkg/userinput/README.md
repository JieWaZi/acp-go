...
- 只监听 loopback；每个会话独立使用随机端口和随机路径，本机用户私有配置中的 Basic 凭据跨进程保持固定，拒绝浏览器 Origin。模型可见的 MCP 服务名固定为 `acp_go_user_input`，端点轮换不改变服务名、工具名、Schema 和鉴权头，避免连续对话破坏提供方前缀缓存。只有当前 Prompt 可以提问；取消立即结束等待，迟到答案不再生效。
