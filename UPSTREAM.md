...
- 历史回放 V1 直接复用已有 event/tool mapper，但按父规格裁剪 upstream 的 history title update、response-item fallback 与非 V1 item；对这些项不另写 mapper。支持的 user text/image/localImage/skill 与 completed agent/tool item 保持 upstream 顺序和 exactly-once 终态；command 保留首条 `tool_call` 和 terminal completion，file/MCP 保留一条带完整展示字段的 completed `tool_call`。多块 user message 在每次 SDK callback 前后重新校验 generation。
